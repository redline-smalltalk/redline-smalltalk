...
public class BlockAnalyser implements NodeVisitor {
	...
	public void useNoOpDelegate() {
//		System.out.println("*** useNoOpDelegate ***");
		BlockAnalyserDelegate delegate = blockAnalyser();
		blockAnalysers.push(new BlockAnalyserNoOpDelegate(this, delegate.className, delegate.packageName, delegate.countOfArguments, delegate.isClassMethod, delegate.containingAnalyser, delegate.thisBlock));
	}

	public void useDelegate(Block block) {
//		System.out.println("*** useDelegate *** " + block);
		blockAnalysers.pop();
		visitEnd(block);
	}
}
